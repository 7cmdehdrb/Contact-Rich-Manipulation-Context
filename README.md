...
이전 논의의 이유가 필요하면 미팅 이력을, 구현 작업을 시작한다면 기술 문서와 코드 현황을 추가로 읽는다.
...
| [출처와 확인 범위](docs/06_SOURCE_REGISTER.md) | 직접 읽은 문서, 대화 기반 정보, 재확보하지 못한 자료와 주장별 근거 |
| [코드·장비 현황](docs/07_CODE_AND_HARDWARE_STATUS.md) | 보고된 구현과 미검증 상태, 과거 PPO 설정, 실행 전 확인할 항목 |
...
- **9월 10일에 보고된 OSC 기반 Sweeping은 사전 검증이다.** 당시 F/T·촉각 관측은 없었고, 최종 Blind 정책이나 Sim-to-Real 성공을 입증하지 않는다.
...
출처 ID는 [출처 목록](docs/06_SOURCE_REGISTER.md)에 정의한다. 문서의 수정 시각보다 **해당 논의가 발생한 날짜와 적용 범위**를 우선한다. 과거 페이지의 ‘최종 제안’이라는 표현도 후속 미팅에서 변경될 수 있다.
...
**출처 범위 주의:** 9월 10·11·14일 Notion 미팅 정리와 연구안 페이지는 직접 확인했다. 과거 첨부 원본 전체, 9월 14일 원본 STT 전체, 실제 학습 코드·체크포인트는 이번 인계 작성에서 확보하지 못했다. 상세한 한계는 출처 목록에 기록했다. 원본 녹취의 무관한 내용과 개인정보는 이 저장소에 복제하지 않는다.
