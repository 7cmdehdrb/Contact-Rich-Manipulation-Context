...
[2026-09-15 제목 선별 보고서](reviews/2026-09-15_iros-2023-2025-tactile-force-torque-rl-title-screening.md)에 2026-09-16 사용자 임시 선정 5편을 기록했다. 아래 ID는 R1–R7과 별개다. 현재 해당 5편 중 3편의 정독이 완료되었으며, 나머지 상태는 [개별 논문 색인](papers/README.md)을 따른다.
...
`papers/`에는 개별 논문 원문 정독 노트를 둔다. 파일명은 **논문 게재연도 + 첫저자 + 논문 주제명**으로 작성한다. R 번호는 기존 조사본 안에서의 식별자이므로 파일명 대신 노트와 색인의 대응 정보로 남긴다. 보완·정정은 같은 파일에서 Git 이력으로 관리한다.

같은 조사본의 오탈자·출처 수정은 기존 파일과 Git 이력으로 관리한다. 조사 기간이나 범위가 달라진 후속 보고서는 새 기준일의 파일로 추가하고 이 색인을 함께 갱신한다. 이전 보고서는 과거 조사 결과로 보존하며, 후속본이 생기면 두 문서를 연결한다. 별도 서지 파일이나 다른 유형의 자료도 실제 자료가 생길 때 목적별 하위 폴더를 추가한다. 빈 폴더나 내용이 중복되는 요약본을 미리 만들지 않는다.
